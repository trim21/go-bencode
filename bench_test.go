	...
	for _, data := range testCase {
		...
		b.Run(data.Name, func(b *testing.B) {
			b.RunParallel(func(pb *testing.PB) {
				for pb.Next() {
					_, err := bencode.Marshal(data.Data)
					if err != nil {
						b.FailNow()
					}
				}
...
func BenchmarkMarshal_ifce_slice_of_type(b *testing.B) {
	...
	for i := 1; i <= 1000; i = i * 10 {
		...
		b.Run(fmt.Sprintf("len-%d", i), func(b *testing.B) {
			...
			bencode.Marshal(D{Value: m}) // warm up

			b.RunParallel(func(pb *testing.PB) {
				for pb.Next() {
					_, err := bencode.Marshal(D{Value: m})
					if err != nil {
						b.FailNow()
					}
				}
...
func BenchmarkMarshal_int(b *testing.B) {
	for i := 10; i <= 1000; i *= 10 {
		...
		b.Run(fmt.Sprintf("marshal int %d", i), func(b *testing.B) {
			...
			b.RunParallel(func(pb *testing.PB) {
				for pb.Next() {
					if _, err := bencode.Marshal(s); err != nil {
						b.Fatal(err)
					}
				}
			})
